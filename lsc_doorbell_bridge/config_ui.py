...
class ConfigurationService:
    ...
    def snapshots(self) -> list[dict[str, Any]]:
        ...
        return [{"id": f"{p.parent.name}/{p.name}", "name": p.name, "mtime": m, "size": size, "url": f"snapshot/{p.parent.name}/{p.name}", "media_source": f"media-source://media_source/local/lsc_doorbell/snapshots/{p.parent.name}/{p.name}"} for m, size, p in items]
