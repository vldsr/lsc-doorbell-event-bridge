...
class Bridge:
    ...
    def _periodic_refresh_loop(self, device_id: str, interval: float) -> None:
        while not STOP.wait(interval):
            self.snapshot_executor.submit(self._capture_media, device_id, "periodic", None)
    ...
    def _schedule_media(self, device_id: str, kind: str) -> None:
        with self.snapshot_lock:
            ...
            timer = threading.Timer(
                self.snapshot_delay,
                lambda: self.snapshot_executor.submit(
                    self._capture_media, device_id, kind, generation
                ),
            )
            timer.daemon = True
